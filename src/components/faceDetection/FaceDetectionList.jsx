...
export default function FaceDetectionList({ data, onSelect, onEdit, onDisable, onEnable, showInactive }) {
  if (!data.length)
      ...
      <table className="w-full text-sm">
        <thead>
          <tr style={{ background: 'rgba(124,58,237,0.08)', borderBottom: '1px solid rgba(124,58,237,0.15)' }}>
            ...
            <th className="px-4 py-3.5 text-left text-xs font-semibold text-purple-400 uppercase tracking-wider">Nombre</th>
            <th className="px-4 py-3.5 text-left text-xs font-semibold text-purple-400 uppercase tracking-wider">Dimensiones</th>
            <th className="px-4 py-3.5 text-left text-xs font-semibold text-purple-400 uppercase tracking-wider">Status API</th>
        ...
        <tbody>
          {data.map(row => (
            ...
            >
              ...
              <td className="px-4 py-3 text-gray-300 text-sm">{row.imageName || '—'}</td>
