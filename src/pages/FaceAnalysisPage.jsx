...
import FaceAnalysisList   from '../components/faceAnalysis/FaceAnalysisList';
import FaceAnalysisDetail from '../components/faceAnalysis/FaceAnalysisDetail';
import FaceAnalysisForm   from '../components/faceAnalysis/FaceAnalysisForm';
import { BrainCircuit, Plus, ArchiveX, Archive } from 'lucide-react';
...
export default function FaceAnalysisPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = showInactive
        ? await faceAnalysisService.findAllInactive()
        : await faceAnalysisService.findAllActive();
      setData(res.data);
    } finally {
      setLoading(false);
    }
  ...
  const handleSave = async ({ imageUrl }) => {
    try {
      if (selected) {
        await faceAnalysisService.update(selected.id, { ...selected, imageUrl });
        alertSuccess('Actualizado', 'El registro fue actualizado correctamente.');
      } else {
        await faceAnalysisService.analyze(imageUrl);
        alertSuccess('Análisis exitoso', 'La imagen fue analizada y guardada.');
      }
      setShowForm(false);
      setSelected(null);
      load();
  ...
  const handleDisable = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await faceAnalysisService.disable(id);
  ...
  const handleEnable = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await faceAnalysisService.enable(id);
    ...
    <div className="p-8">
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-white glow-text">Face Analysis</h2>
      ...
      {detail && <FaceAnalysisDetail record={detail} onClose={() => setDetail(null)} />}
